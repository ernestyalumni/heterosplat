 ...
 *   1. Extracted the two __device__ helpers (sh_coeffs_to_color_fast and
 *      its VJP) and the two templated __global__ kernels verbatim from
 *      upstream gsplat's SphericalHarmonicsCUDA.cu. The accompanying
 *      launch_* host functions and AT_DISPATCH_FLOATING_TYPES dispatch are
 *      NOT carried over -- heterosplat replaces them with raw-pointer
 *      launchers in ../../Heterosplat/SphericalHarmonics.cu.
 *   2. Replaced ATen's gpuAtomicAdd (from <ATen/cuda/Atomic.cuh>) with the
 *      CUDA-native atomicAdd. gpuAtomicAdd is a thin ATen wrapper around
 *      atomicAdd for the float case used here, so the substitution is
 *      bit-equivalent.
 *
 * Implementation reference (kept from upstream):
...
__global__ void spherical_harmonics_bwd_kernel(
    ...
    if (v_dirs != nullptr) {
        // NOTICE: heterosplat substitution -- gpuAtomicAdd (ATen) ->
        // atomicAdd (CUDA built-in). Bit-equivalent for float.
