...
| `RasterizeToPixels3DGSKernels.cuh` | extracted from `cuda/csrc/RasterizeToPixels3DGSFwd.cu` + `cuda/csrc/RasterizeToPixels3DGSBwd.cu` | **patched** — fwd + bwd `__global__` templates kept; ATen launchers, `GSPLAT_BUILD_3DGS` guard, `Rasterization.h` include, and `GSPLAT_FOR_EACH` instantiations dropped; `gpuAtomicAdd` → `atomicAdd` |
